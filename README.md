...
- **For experienced Arduino users:** You might find the [Quick Start](https://github.com/dwblair/Riffle-Getting-Started#quick-start) section sufficient to get up and running quickly.  In general, the Riffle behaves like an Arduino Uno in the Arduino IDE; but its USB-Serial chip (the CH340) **requires a special driver** on Mac and Windows (Linux supported by default). See [Software prep](test) below. 

- **For those who would prefer step-by-step instructions** for setting up your Riffle, please check out the [Step-by-Step Instructions](test).
